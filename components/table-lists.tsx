...
export default function TableLists<T extends WithKey>(props: Readonly<Props<T>>) {
          ...
          data.map((item: T) => (
            <TableRow key={item.id} className="hover:bg-gray-50">
              {columns.map((column) => (
                ...
                <TableCell key={column.key} className="whitespace-nowrap px-3 py-2.5 text-xs text-gray-500 capitalize">
                  {
                    column.render
                      ? column.render(item)
                      : item[column.key]
