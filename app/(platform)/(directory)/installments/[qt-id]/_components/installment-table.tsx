...
export default function InstallmentTable({ installments: initialInstallments, quotationId }: InstallmentTableProps) {
    ...
    const currentTotal = form.watch("installments").reduce((sum, item) => sum + item.amountWithVat, 0);
    const originalTotal = initialInstallments.reduce((sum, item) => sum + item.amountWithVat, 0);
                                ...
                                <div className="flex items-center space-x-2 px-3 py-2 bg-yellow-50 border border-yellow-200 rounded-md">
                                    <div className="text-sm text-yellow-800">
                                        ...
                                        <div className="mt-1 flex items-center justify-between space-x-4">
                                            ...
                                            <div className={`font-medium ${totalDifference > 0 ? 'text-green-700' : 'text-red-700'}`}>
                                                ผลต่าง: {totalDifference > 0 ? '+' : ''}{formatCurrency(totalDifference)}
                                            </div>
                                        </div>
                                    </div>
                                </div>
